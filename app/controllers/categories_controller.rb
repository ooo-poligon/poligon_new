  ...
  def show
    ...
    Product.available.where(category_id: params[:id]).each do |product|
      ...
      if product.currency_id == 1
        if !product.nil? && (product.stock - product.reserved) > 0
          one_product_array.push(product, product.base_price * (@courseEuro + (@courseEuro / 100) * addCBR.text_value.to_f).round(2), 1)
        else
          one_product_array.push(product, product.base_price * (@courseEuro + (@courseEuro / 100) * addCBR.text_value.to_f).round(2), 0)
        end
      elsif product.currency_id == 2
        if product.nil? && product.stock > 0
          one_product_array.push(product, product.rub_base_price(2), 1)
        else
          one_product_array.push(product, product.rub_base_price.round(2), 0)
        end
      end
